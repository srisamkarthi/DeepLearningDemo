...
def conv_net(x_dict, n_classes, dropout, reuse, is_training):
    ...
    with tf.compat.v1.variable_scope('ConvNet', reuse=reuse):
        ...
        conv1 = tf.compat.v1.layers.conv2d(x, 32, 5, activation=tf.nn.relu)
        # Max Pooling (down-sampling) with strides of 2 and kernel size of 2
        conv1 = tf.compat.v1.layers.max_pooling2d(conv1, 2, 2)

        # Convolution Layer with 64 filters and a kernel size of 3
        conv2 = tf.compat.v1.layers.conv2d(conv1, 64, 3, activation=tf.nn.relu)
        # Max Pooling (down-sampling) with strides of 2 and kernel size of 2
        ...
        out = tf.compat.v1.layers.dense(fc1, n_classes)
